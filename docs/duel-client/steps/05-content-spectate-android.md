...
- [ ] Step 4.5 staging web URL live; CORS/WSS lessons captured in exit notes
...
- **Production web (ADR-014):** desktop-responsive board/lobby polish; production (or prod-like) Vercel domain; spectate share URLs if spectate ships; keep planner and duel as **separate** Vercel projects.
...
- Replacing Expo web with a from-scratch Vite duel SPA.
- Hosting Colyseus on Vercel serverless.
- First-time staging web deploy (that is Step 4.5).
...
| Production web polish | `mobile/**` layout + Vercel prod project/domain notes |
...
4. **Web:** widen layout for ≥1024px; keyboard/pointer affordances; production env pointing at prod WSS; feature-flag real names/art if required; optional CDN for card images.
...
- [ ] Production (or prod-like) web URL: desktop layout usable; duel completes; planner Vercel project still separate/unbroken.
